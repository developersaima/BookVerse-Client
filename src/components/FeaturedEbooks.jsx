  ...
  return (
    <section className="py-20 px-6 max-w-7xl mx-auto border-t border-slate-50">
      <div className="text-center mb-16">
        <h2 className="text-3xl font-extrabold text-slate-950">Featured <span className="text-[#00a851]">Ebooks</span></h2>
      ...
      {loading ? (
        ...
        <div className="p-10 border border-slate-100 bg-slate-50/50 rounded-2xl max-w-sm mx-auto text-center">
          <p className="text-slate-400 text-sm font-medium">No published ebooks found.</p>
        </div>
      ) : (
        <motion.div 
          initial="hidden"
        ...
        >
          {books.map((book) => (
            <motion.div 
              ...
              variants={fadeIn}
              whileHover={{ y: -6 }}
              className="group border border-slate-100 bg-white hover:border-[#00a851]/30 hover:shadow-xl hover:shadow-slate-100/50 rounded-2xl overflow-hidden transition-all duration-300"
            >
              <figure className="aspect-[4/5] bg-slate-50 overflow-hidden relative">
                <img 
              ...
              <div className="p-6">
                ...
                <h3 className="text-lg font-bold mt-4 text-slate-800 group-hover:text-[#00a851] transition-colors truncate">
                  {book.title}
                </h3>
                <p className="text-xs text-slate-400 mt-1">By {book.writerName || "Anonymous"}</p>
