...
export default function AdminDashboard({ activeTab }) {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const [analyticsRes, usersRes, ebooksRes] = await Promise.all([
        ...
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/ebooks`, {
          headers,
        }),
      ]);
  ...
  const handleEbookDelete = async (ebookId, title) => {
    ...
    try {
      const headers = await getAuthHeaders();
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/ebooks/${ebookId}`,
        {
          method: "DELETE",
          headers,
        },
      ...
      if (res.ok) {
        ...
        setEbooks(ebooks.filter((b) => b._id !== ebookId));
        await fetchDashboardData();
      } else {
        const error = await res.json();
  ...
  const handleToggleEbookStatus = async (ebookId, currentStatus) => {
    setActionLoading(true);
    try {
      const headers = await getAuthHeaders();
      const newStatus =
      ...
      if (res.ok) {
        toast.success(
          `Ebook ${newStatus === "published" ? "published" : "unpublished"}`,
        );
        setEbooks(
          ebooks.map((b) =>
            b._id === ebookId ? { ...b, status: newStatus } : b,
          ),
        );
        await fetchDashboardData();
      } else {
        ...
        <div className="bg-base-100 rounded-2xl border border-base-200 shadow-sm overflow-hidden">
          <div className="p-5 border-b border-base-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-base-content/40">
                Manage all ebooks on the platform
              </p>
            </div>
            <div className="flex items-center gap-3">
                ...
                {ebooks.map((ebook) => (
                  ...
                  >
                    ...
                    <td className="text-right font-bold text-emerald-600">
                      ${ebook.price?.toFixed(2) || "0.00"}
                    </td>
                    <td>
                      <span
                        className={`badge ${ebook.status === "published" || ebook.status === "available" || ebook.status === "In Stock" ? "badge-success" : ebook.status === "sold" || ebook.status === "Sold Out" ? "badge-error" : "badge-warning"} gap-1`}
                      >
                        {ebook.status === "published" ||
                        ebook.status === "available" ||
                        ebook.status === "In Stock" ? (
                    ...
                    <td>
                      <div className="flex gap-2 justify-center">
                        <button
                          onClick={() =>
                            handleToggleEbookStatus(ebook._id, ebook.status)
                          }
                        ...
                        <button
                          onClick={() =>
                            handleEbookDelete(ebook._id, ebook.title)
                          }
                          className="btn btn-sm btn-error text-white gap-1 hover:scale-105 transition-transform"
                          disabled={actionLoading}
                        >
              ...
              <tbody>
                {transactions.map((transaction, index) => (
                  ...
                  >
                    ...
                    <td>
                      <span
                        className={`badge ${transaction.type === "purchase" ? "badge-success" : "badge-info"} gap-1`}
                      >
                    ...
                    <td>
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-base-content/40">
                          {transaction.buyerEmail || transaction.email || "N/A"}
                        </span>
                      </div>
                    </td>
                    <td className="text-right font-bold text-emerald-600">
