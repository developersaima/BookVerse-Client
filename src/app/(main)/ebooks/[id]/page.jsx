...
export default function EbookDetailsPage({ params }) {
  ...
  const fetchBookmarkStatus = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const isBookmarkedNow = Array.isArray(bookmarks) && bookmarks.some(b => b._id === id);
    ...
    return (
      <div className="min-h-screen bg-base-200 flex flex-col justify-center items-center px-4">
        <div className="text-center space-y-4 max-w-md bg-base-100 p-8 rounded-2xl shadow-xl">
          ...
          <h2 className="text-2xl font-bold text-base-content">Ebook Not Found</h2>
          <p className="text-base-content/70 text-sm">The ebook you are looking for might have been removed.</p>
  ...
  const isSold = ebook.status?.toLowerCase() === "sold" || ebook.status === "Sold Out";
  const hasAccess = hasPurchased || isWriter;
  const formattedDate = ebook.createdAt 
    ? new Date(ebook.createdAt).toLocaleDateString('en-US', { day: 'numeric', month: 'long', year: 'numeric' }) 
    ...
    <div className="min-h-screen bg-base-200 py-8 px-4 md:px-8 lg:px-12">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <Link href="/" className="btn btn-ghost gap-2 hover:bg-green-50 hover:text-green-700">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {bookmarkPending ? "Processing..." : isBookmarked ? "Bookmarked" : "Bookmark"}
            </button>
          </div>
        </div>
          ...
          <div className="lg:col-span-4 xl:col-span-3">
            <div className="card bg-base-100 shadow-xl overflow-hidden sticky top-24">
              <figure className="relative aspect-[3/4] bg-base-200">
                <img 
                  src={ebook.coverImage || ebook.image || "/placeholder-cover.jpg"} 
                  alt={ebook.title} 
              ...
              <div className="card-body p-4">
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div className="bg-base-200 rounded-lg p-3 text-center">
                    <span className="block text-xs text-base-content/60 font-medium uppercase tracking-wider">Status</span>
                    <span className={`font-bold ${isSold ? 'text-error' : 'text-success'}`}>
                      {isSold ? "Sold Out" : "In Stock"}
                    </span>
                  </div>
                  <div className="bg-base-200 rounded-lg p-3 text-center">
                    <span className="block text-xs text-base-content/60 font-medium uppercase tracking-wider">Format</span>
                    <span className="font-bold text-base-content">Digital</span>
                  </div>
                  <div className="bg-base-200 rounded-lg p-3 text-center">
                    <span className="block text-xs text-base-content/60 font-medium uppercase tracking-wider">Pages</span>
                    <span className="font-bold text-base-content">{ebook.pageCount || "N/A"}</span>
                  </div>
                  <div className="bg-base-200 rounded-lg p-3 text-center">
                    <span className="block text-xs text-base-content/60 font-medium uppercase tracking-wider">Language</span>
                    <span className="font-bold text-base-content">{ebook.language || "English"}</span>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="card-body p-6 md:p-8">
                ...
                {ebook.language && (
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-base-content/70">Language: <span className="font-medium text-base-content">{ebook.language}</span></span>
                  </div>
                )}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-green-50 rounded-2xl p-6 border border-green-100">
                  <div>
                    <span className="block text-xs font-semibold text-green-700 uppercase tracking-wider">Price</span>
                    <div className="flex items-baseline gap-2">
                      <span className="text-4xl font-black text-green-700">${ebook.price}</span>
                      {ebook.originalPrice && (
                        <span className="text-sm text-base-content/40 line-through">${ebook.originalPrice}</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                    ...
                    {isWriter && (
                      <button className="btn btn-ghost gap-2 flex-1 sm:flex-none text-green-600 border-green-200 hover:bg-green-50" disabled>
                    ...
                    {!hasPurchased && !isWriter && !isSold && (
                      ...
                      >
                        ...
                        {bookmarkPending ? "Processing..." : isBookmarked ? "Bookmarked" : "Save"}
                      </button>
                    )}
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-4 pt-2 text-xs text-base-content/40">
                  ...
                  <span>•</span>
                  <span className="flex items-center gap-1">
                    <span className="text-green-600">✓</span> Instant access after purchase
                  </span>
                  <span>•</span>
                  <span className="flex items-center gap-1">
